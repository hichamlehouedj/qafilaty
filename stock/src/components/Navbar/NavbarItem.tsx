...
const NavbarItem: FC<Props> = (props) => {
    return (
        <Tooltip title={(<span style={{ fontSize: 12 }}>{props.title || ""}</span>)} placement="right"
            PopperProps={{
                modifiers: [
                    {
                        name: "offset",
        ...
        >
            <>
            <Box {...props} className={clsx("List-item", { selected: props.selected })}>
                <CardActionArea>
                    <Box className={clsx("List-item--innerItem selected")}>
                        {props.children}
                    </Box>
                </CardActionArea>
            </Box>
