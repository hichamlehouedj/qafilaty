...
import {Alert, Chip, Snackbar, Stack, Typography} from "@mui/material";
...
const ShipPhyisicalScannerModal = (props: Props) => {
    ...
    useEffect(() => {
        ...
        if (props.open) {
            ...
            document.addEventListener("scan", phisicalScannerHandler as any);
            setIsScanning(true);
        } else {
            setScannedShipments([]);
            setIsScanning(false);
        ...
        return () => {
            document.removeEventListener("scan", phisicalScannerHandler as any);
        };
    }, [props.open]);

    const phisicalScannerHandler = function (e: any, sChar: any) {
        let segments: any = e.detail?.scanCode?.match(/.{1,3}/g);
        let shipmentCode: any =  /*"Env-5912487" ||*/  String.fromCharCode(...segments);
        e?.preventDefault();
            ...
            <Snackbar open={scanAlert.open} autoHideDuration={6000} anchorOrigin={{ vertical: "top", horizontal: "center" }}>
                ...
                >
                    <Stack direction="row" gap="14px" alignItems={"center"}>
                        ...
                        {scanAlert?.status == "info" && (
                            <Typography variant="xs">في إنتظار عملية المسح!</Typography>
                        )}
                ...
                footer={
                    <>
                        <Stack direction="row" justifyContent={"space-between"} width="100%">
                            <Button variant="contained" color="secondary" type="submit" form="add_request"
                                startIcon={isScanning ? <X /> : <ScanLine />}
                                onClick={() => setIsScanning(!isScanning)}
                            >
                                {isScanning ? "إنهاء المسح" : "إبدا المسح"}
                            </Button>
...
export default ShipPhyisicalScannerModal;