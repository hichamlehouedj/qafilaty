...
import { useGetStatisticsStock } from "../graphql/hooks/statistics";
...
const chartOption = {
    fill: {
        gradient: {shadeIntensity: 1, opacityFrom: 0.5, opacityTo: 0.5,},
    },
    grid: {
        show: false,
        padding: {left: 4, right: 3, bottom: 6, top: 0},
    },
...
export const Statistics = (props: Props) => {
    ...
    const [dataMony, setDataMony] = useState<number[]>([0, 0, 0, 0, 0])
    const [dataAmount, setDataAmount] = useState<number[]>([0, 0, 0, 0, 0])

    let theme = useTheme();
    let statisticsData = useGetStatisticsStock({
    ...
    let areaOptions01 = {
        series: [
            { name: "محفظة", data: dataMony },
        ],
        options: {colors: [theme.palette.primary.main], ...chartOption},
    } as ChartProps;

    let areaOptions02 = {
        series: [
            { name: "العمولات", data: dataAmount },
        ],
        options: {colors: [theme.palette.primary.main], ...chartOption}
    } as ChartProps;
    ...
    useEffect(() => {
        if (statisticsData) {
            let money: number[] = []
            let amount: number[] = []
            statisticsData?.chartMoney?.map((chart: any) => money.push(chart.total))
            setDataMony(money)

            statisticsData?.chartAmount?.map((chart: any) => amount.push(chart.total))
            setDataAmount(amount)
        }
    }, [statisticsData]);

    console.log("dataMony ", dataMony)
    if (!statisticsData)
        return (
    ...
    return (
        <>
            <Head><title>إحصائيات | قافلي</title></Head>
                                    ...
                                    <Box flex={"1"} marginRight="-28px">
                                        <Stack height="100%" direction={"row"} alignItems={"center"} gap="30px">
                                            <Stack gap="12px">
                                                ...
                                                <Typography variant="base" fontWeight={500} color={grey[500]}>أموال في الفرع</Typography>
                                            </Stack>
                                    ...
                                    <Box height="80px">
                                        <Stack height="100%" direction="row">
                                            <Box flex="1" paddingTop={"4px"} /* paddingLeft={"20px"} */ >
                                                <Stack gap="14px">
                                                    <Typography variant="sm" color={grey[500]}>أموال في الطريق</Typography>
                                            ...
                                            <Box flex="1" paddingTop={"4px"} paddingLeft={"20px"}>
                                                <Stack gap="14px">
                                                    <Typography variant="sm" color={grey[500]}>أموال مدفوعة</Typography>
                                                    <Typography variant="sm" color={grey[800]}>
                            ...
                            <Box bgcolor={"#FFF"} height="114px" padding={"25px 28px"} borderRadius="6px">
                                <Stack direction="row" columnGap="24px" alignItems="center">
                                    <Box borderRadius={"50%"} width="64px" height="64px"
                                        ...
                                        <BoxIcon size={36} color={theme.palette.primary.main}/>
                                    </Box>
                                    <Stack gap="4px">
